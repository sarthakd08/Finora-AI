      ...
      <header className="border-b bg-white/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-900">{call.title}</h1>
              <p className="text-sm text-slate-600">with {call.agentName}</p>
            </div>
          </div>
        </div>
      </header>
        ...
        <Card className="mb-8 border-none shadow-lg bg-white/80 backdrop-blur-sm">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-xl mb-2">Consultation Summary</CardTitle>
                <CardDescription>{call.summary}</CardDescription>
              </div>
              <Badge 
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="flex items-center gap-2 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">{new Date(call.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 text-sm">
                ...
                <>
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{call.feedback.rating}.0 / 5.0</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <CheckCircle2 className="w-4 h-4 text-green-500" />
                    <div>
                      <p className="text-xs text-slate-500">Helpfulness</p>
                      <p className="font-medium">{call.feedback.helpfulness}/5</p>
                    </div>
                  </div>
                </>
              )}
            ...
            {call.report && (
              <>
                ...
                <Card className="border-none shadow-lg bg-white/80 backdrop-blur-sm">
                  ...
                  <CardContent>
                    <p className="text-slate-700 leading-relaxed">{call.report.overview}</p>
                  </CardContent>
                </Card>
                    ...
                    <ul className="space-y-3">
                      {call.report.keyPoints.map((point, index) => (
                        <li key={index} className="flex items-start gap-3">
                          <span className="w-6 h-6 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center text-sm font-semibold flex-shrink-0 mt-0.5">
                            {index + 1}
                          </span>
                  ...
                  <CardContent>
                    <ul className="space-y-3">
                      {call.report.actionItems.map((item, index) => (
                        <li key={index} className="flex items-start gap-3 p-4 rounded-lg border-l-4 border-orange-500 bg-orange-50">
                ...
                <CardContent className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="bg-slate-50 rounded-lg p-4 border-l-4 border-blue-600">
                      <p className="text-slate-700 leading-relaxed italic">&quot;{call.feedback.comment}&quot;</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
